...
import { deanClickApprovedResultSheetsCollectionView } from '../../store/reducers/DeanNavigationSlice'

function DeanApprovedResultCollectionResult() {
    const [currentSheet, setCurrentSheet] = useState(null);
    const [students, setStudents] = useState([]);
    const [loading, setLoading] = useState(true); // Optional: Loading state
    const pendingSheetId = useSelector((store) => store.deanNavigationSlice?.pendingDeanSheetId);
    const resultsSheets = useSelector((store) => store.deanNavigationSlice?.studentCollectionItemResultsSheets || []);
...
export default DeanApprovedResultCollectionResult
