...
import { setStudentHistoryResultsSheets } from '../../store/reducers/DptSecretaryNavigationSlice';

function DptApprovedResultList() {
  ...
  const fetchPendingSubjectResultSheets = async (id) => {
      try {
          const response = await axios.get(`${host}/api/results/secApproved/${id}`);
...
export default DptApprovedResultList
