...
import { clickViewResult, setStudentResultsSheets } from '../../store/reducers/LectureNavigationSlice';
import { setPendingHodSheetId } from '../../store/reducers/HODNavigationSlice';
import { setResultsSheets, setStudentPendingResultsSheets } from '../../store/reducers/LectureNavigationSlice';


function ApprovelRequestResultList() {
    ...
    useEffect(() => {
      const getResultSheets = async () => {
          try {
              ...
              dispatch(setStudentPendingResultsSheets (data));
              // dispatch(setResultSheets(data));
  ...
  const fetchPendingSubjectResultSheets = async (id) => {
      try {
          const response = await axios.get(`${host}/api/results/getPending/${id}`);
        ...
        <div>
            <div className='flex flex-col items-center'>
                <div className='mt-3 w-5/6 flex items-center justify-between'>
                    <h3 className='text-xl text-primary-txt'>Approval Requsested Results</h3>
                </div>
                <div className='w-full flex justify-start'>
                    <div className='ml-20 mt-3 px-8 py-2 flex items-center justify-center bg-approval-btn-bg'>Requested</div>
                </div>
...
export default ApprovelRequestResultList
